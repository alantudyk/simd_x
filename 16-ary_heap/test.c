...
int main(void) {
    ...
    const __m256i
        P2 = _mm256_set_epi32(6, 7, 4, 5, 2, 3, 0, 1),
    ...
    const __m256i b = _mm256_loadu_si256((void *)(x + 8));

    __m256i m = _mm256_min_epi32(a, b), p;

    p = _mm256_permutevar8x32_epi32(m, P2);
    m = _mm256_min_epi32(m, p);

    p = _mm256_permutevar8x32_epi32(m, P4);
    m = _mm256_min_epi32(m, p);

    p = _mm256_permutevar8x32_epi32(m, P8);
    m = _mm256_min_epi32(m, p);
